...
const load = async () => {
  ...
  function checkAttributes(element, attributes, matches) {
    const foundAttr = attributes.find((_attribute) => {
      const value = element.getAttribute(_attribute);
  ...
  function getBestInput(names, exclusionNames, types) {
    ...
    // this list includes duplicates, but we only use the first one
    // we find that matches, so there's no need to dedupe
  ...
  function addFocusListener(element, credentials) {
    ...
    function addOption(parent, username) {
      ...
      suggestionItem.addEventListener('click', () => {
        const selectedCredentials = credentials.filter((el) => el.username === username)[0];
  ...
  function requestAutofill() {
    if (getBestUsernameField() && getBestPasswordField()) {
      ipcRenderer.send('password-autofill', document.location.hostname);
    }
  }
  ...
  function createUnlockButton(input) {
    ...
    button.innerHTML = escapeHTMLPolicy.createHTML(keyIcon);
